...
function comparePrices(
  oldPrices: ShopItem["prices"],
  newPrices: ShopItem["prices"]
): boolean {
  const oldEntries = Object.entries(oldPrices).filter(
    ([_, price]) => price !== undefined
  );
  const newEntries = Object.entries(newPrices).filter(
    ([_, price]) => price !== undefined
  );
...
export function NewItem({ item }: { item: ShopItem }) {
  const renderStock = () => {
    if (item.stockRemaining === 0) {
      return (
        <>
          <Warning /> <b>Out of stock</b>
        </>
      );
    } else if (typeof item.stockRemaining === "number") {
      return (
        <>
          <Warning /> <b>Stock:</b> {item.stockRemaining} items available
        </>
      );
    } else {
      return (
  ...
  return (
    <Blocks>
      <Header>
        <New /> {item.title} (<Shells /> {formatPrices(item.prices)})
      </Header>
      <Section>
      ...
      {item.imageUrl ? (
        <Image src={item.imageUrl} alt={`Image for ${item.title}`} />
      ) : null}
    </Blocks>
  );
}
...
export function DeletedItem({ item }: { item: ShopItem }) {
  return (
    <Blocks>
      <Header>
        <Trash /> {item.title} (<Shells /> {formatPrices(item.prices)})
      </Header>
      <Section>
        {item.description && item.description !== "" ? (
          <i>{item.description}</i>
...
export function UpdatedItem({
  ...
  const renderStock = () => {
    if (stockChanged) {
      const oldStock =
        oldItem.stockRemaining === 0
          ? "Out of stock"
          : typeof oldItem.stockRemaining === "number"
          ? `${oldItem.stockRemaining} items`
          : "Unlimited";

      const newStock =
        newItem.stockRemaining === 0
          ? "Out of stock"
          : typeof newItem.stockRemaining === "number"
          ? `${newItem.stockRemaining} items`
          : "Unlimited";
      ...
      if (newItem.stockRemaining === 0) {
        ...
        return (
          <>
            <Warning /> <b>Stock:</b> {newItem.stockRemaining} items available
          </>
        );
      } else {
        return (
  ...
  return (
    <Blocks>
      <Header>
        ...
        <Shells />{" "}
        {priceChanged
          ? `${formatPrices(oldItem.prices)} → ${formatPrices(newItem.prices)}`
          : formatPrices(newItem.prices)}
        )
      </Header>
      <Section>
        {descChanged
          ? `${oldItem.description || "_no description_"} → ${
              newItem.description || "_no description_"
            }`
          : newItem.description}{" "}
        <br />
        {renderStock()}
      ...
      {imageUrlChanged && oldItem.imageUrl ? (
        <Image src={oldItem.imageUrl} alt={`Old image for ${newItem.title}`} />
      ) : null}
      {newItem.imageUrl ? (
        <Image src={newItem.imageUrl} alt={`New image for ${newItem.title}`} />
      ) : null}
    </Blocks>
  );
}
...
export function UsergroupPing({ usergroupId }: { usergroupId: string }) {
  return (
    <Blocks>
      <Context>
        pinging <a href="#">@channel</a> ·{" "}
        <a href="https://go.skyfall.dev/som-monitor">
          <Star /> star the repo!
        </a>{" "}
        ·{" "}
        <a href="https://go.skyfall.dev/converge">
          <Robot /> discord/slackbot ysws
        </a>
      </Context>
    </Blocks>
  );
